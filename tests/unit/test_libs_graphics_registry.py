...
def test_registry_noframe_indexing(mock_properties, mock_configurations):
    ...
    mock_configurations = dataclasses.replace(
        ...
        recipes=dataclasses.replace(
            mock_configurations.recipes,
            objects=ObjectRecipe(chests=Recipe(frame=FrameRecipe.NONE))
        )
    )
...
def test_registry_spriteframe_indexing(mock_properties, mock_configurations):
    ...
    mock_configurations = dataclasses.replace(
        ...
        recipes=dataclasses.replace(
            mock_configurations.recipes,
            sheets=SheetRecipe(sprites=Recipe(frame=FrameRecipe.SPRITE))
        )
    )
...
def test_registry_index_frame_indexing(mock_properties, mock_configurations):
    ...
    with patch('libs.graphics.registry.os.walk') as mock_walk, \
        ...
        assert "items_sheet-shield" in registry._frames
        assert "items_sheet-potion" in registry._frames
        
        data_shield = registry.image("items_sheet-shield")
