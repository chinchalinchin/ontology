...
A Menu has focus. Focus is a field that points to the currently active Widget.
...
- `frames: Optional[List[str]]`: List of frame keys.
...
The amount of text a Page is capable of rendering at once is calculated at runtime. This is used to populate the `pages` and `pagesize` fields.

**State: PageState**

- `text: str`
- `current: str:`  = `text[0 : max(pagesize, len(text)]`
- `pages`
- `pageindex: int` = 0
- `pagesize: int`
...
- `more(): return text[-len(current) : ] != current`
- `scroll(): current = text[pageindex*pagesize : max(pageindex*(pagesize+1), len(text))]`
