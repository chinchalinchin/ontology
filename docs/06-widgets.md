...
1. **The Render List:** A flat list of `Asset` objects with absolute screen coordinates (for `Screen.draw()`).
2. **The Traversal Graph:** A directed graph (adjacency list) mapping each interactive `Button.name` to its logical neighbors (`NORTH`, `SOUTH`, `EAST` and `WEST`).
