...
Keys are used to map assets to images loaded into the [registry](./00-overview.md%23registry), to ensure each Asset is only loaded into the memory once, no matter how many times it is rendered in a single frame. In other words, keys uniquely identify a physical Asset, but not in-game objects.
...
!!! note "Layers"
    The concept of a Layer is defined more explicitly in [World documentation](./00-overview.md%23world). It suffices to think of Layers as floors in a house, i.e. where each floor has the same area and similar topology, but occupies a different height. In-game, Layers are traversed by the Player interacting with Doors.
...
- *Mutable*, *Inanimate* Assets
    ...
    - Object: Gate, State: Position, Layer, Switch, Key
    - Object: Plate, State: Position, Layer, Switch, Key
- *Mutable*, *Inanimate* Assets
...
- *Mutable*, *Animate* Assets
    ...
    - Sheet: Sprite, State: Position, Layer, Frame, Direction, Action
