...
As a simple example, a Plot state might be defined to change conditional on the existence of Sprite (i.e. whether a character is alive or dead). In the first state, a Sprite might have access to a certain Communication in its Memory (See [Sprite documentation](./02-sprites.md) for more details on the internal mechanics of Sprites), while in the other a different Communication is present.