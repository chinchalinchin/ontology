...
Equipment is a stateless Sheet Asset. It is not deployed onto the Board in a fashion like other Assets. Instead, only properties are supplied for Equipment. This allows the Registry to index and load all Equipment Sheets. However, since Equipment does not possess a Recipe and therefore does not possess State, it is not instantiated by the game engine when the state directory is traversed during the bootstrapping. Instead, Equipment frame keys are generated by SpriteFrames when Sprites have Equipment slotted into their `sprite.state.inventory.equipment`, e.g.
