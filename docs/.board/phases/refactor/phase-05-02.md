...
- [x] Create an `EngineContext` dataclass to pass along the assembly line.
...
- [x] Move `Decomposer`, `Provider`, and `Cradle` into the new `app.services.generators` package.
    - [x] Update their constructors to accept only the specific properties and recipes they need from the `EngineContext`, rather than the whole schema.
- [x] Create an `EngineBuilder` class that breaks the `Orchestrator`'s monolithic `init()`, `migrate()`, and `inject()` methods into discrete, single-responsibility steps.
