...
*Objective*: Support continuous integration without integer truncation failure.

* [ ] **Subtask**: Update `libs.core.models.Position` to track `cdef public double rx, ry` for accumulating fractional pixel movement.
...
**6. Task: Overhaul `MotionMechanics` (Time Integration)**
...
* [ ] **Subtask**: Query `Board.tile_at()` using the Asset's center point. Apply the tile's friction coefficient to the Asset's velocity.
* [ ] **Subtask**: Integrate Position: `rx += vx * delta`. When `rx >= 1.0` or `rx <= -1.0`, cast to `int`, apply to `Position.x`, and subtract from `rx`. Repeat for `Y`.
...
* [ ] **Subtask**: Distribute the `overlap_x` and `overlap_y` positional correction shifts strictly proportionally to the inverse mass ratio.
* [ ] **Subtask**: Apply 1D perfectly inelastic momentum formulae to the `Velocity` vectors of both objects post-separation.
