...
- [x] Initialize `Engine` in `Orchestrator.ignite()`
...
* [x] Refactor the `Frame.key()` abstract method to `Frame.keys()`, updating the return type from `str` to `List[str]`.
* [x] Update `SingleFrame`, `IterableFrame`, and `StateFrame` implementations to wrap their resolved string in a list.
* [x] Update `Registry.data()` to anticipate iterative querying if necessary, or ensure `Screen.draw()` handles the list iteration.
...
* [x] Create a `SpriteFrame` class inheriting from `StateFrame`.
* [x] Override `keys(id, state)` to initialize a list, starting with the base Persona frame key: `{persona}-{state.action}-{state.direction}-{state.frame}`.
* [x] Iterate over the active equipment keys in `state.inventory.equipment` in strict Z-index order (e.g., Base -> Armor -> Utility -> Tool -> Weapon).
* [x] For each active equipment piece, blindly format and append the equipment frame key: `{equipment_key}-{state.action}-{state.direction}-{state.frame}`.
...
* [x] In `Screen.draw()`, update the loop to iterate over the `List[str]` returned by `asset.frame.keys(asset.id, asset.state)`.
* [x] For each key in the returned list, query `registry.data(key)`.
* [x] If the texture exists, flatten it into the primitive tuple and append it to `active_assets`.
* [x] Ensure `active_assets.sort(key=lambda x: x[6] + x[8])` remains untouched, relying on Python's stable sort to preserve the Z-index order yielded by `SpriteFrame`.
