...
* **Price Transmission (Gossip):** You already have a `communicate` Disposition and a `memory.communications` buffer. When two Sprites enter the `communicate` Disposition within a certain radius, the `CommunicationMechanic` doesn't just swap dialogue; it swaps the latest transaction values from their `price_book`.
...
2. The `barter` Disposition & Transactions


* **The Intent:** A Sprite realizes it needs Wood. Its Disposition matrix evaluates its inventory, transitioning it from `idle` -> `find` (Goal: Sprite with Wood). Once within `parameters.vision.radius` of a target Sprite, it transitions to `barter`.
* **The CommerceMechanic:** Add a `CommerceMechanic` to `Board.play()`. This mechanic specifically queries `board.sprites` for Sprites in the `barter` Disposition.
* **The Handshake:**
    - Sprite A enters `barter` with target Sprite B.
    - Sprite B's Disposition matrix evaluates the interaction. If Sprite B feels safe and is interested, it also transitions to `barter`.
    ...
    - The Disposition transitions to `return` or `idle`.
...
* **The `build` Extension:** When a Sprite acquires enough materials (e.g., 50 Wood, 10 Stone), its overarching `Goal` shifts. Its Disposition changes to `wander` to find an empty plot (using `CollisionMechanics` to ensure the space is clear), and then enters a `build` Extension.
* **Asset Spawning:** While in the `build` Extension, a new `Mutable, Inanimate Asset` (e.g., `Foundation`) is appended to the `Board`. As the game loop progresses, the Sprite consumes its inventory, and the `AnimationMechanics` advances the `Foundation`'s frame until it becomes a `House`.
...
dispositions:
  ...
  threaten:
    transitions:
      ...
      - next: idle
        condition:
          - not sprite.memory.goal.target.inventory.loot['wood']
          
      # If the target doesn't submit, escalate to attack
      - next: attack
        condition:
          - sprite.memory.goal.target.disposition != 'escape'
...
6. The NPC's Disposition matrix sees that it cannot afford the Wood, but its `motivation` is `survival`. The matrix kicks the NPC into `threaten`.
...
8. Violence erupts. Other Sprites nearby with `kinship` motivations see their ally in `attack` disposition and cascade into the fight. A full-scale border war emerges purely from inflation.
