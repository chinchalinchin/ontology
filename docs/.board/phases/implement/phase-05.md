...
* [~] *Define States:* Implement `TraversalState`, `MeterState`, and `DisplayState` in `app.models.state`. 
    * [ ] Implement scrolling methods for `DisplayState`. `content` is either a string to be rendered that needs to be sliced, or it is a list of Icon keys. Leave TODO placeholders for the Icon handling, as the spec has not been completed.
...
* [~] *Implement Scrolling*: Implement `app.game.menus.controllers.scroll.ScrollController` (Modifies `DisplayState.pageindex`).
    * [~] Implement `ScrollController.select()`. It must parse the selection binding (e.g., `SCROLLUP`), locate the target widget via the selector binding and call `scroll()`
