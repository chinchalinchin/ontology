...
- Define the traversal graph construction algorithm in pseudocode.
...
**5. Traversal Graph Spatial Resolution**

The backlog proposes building a `TraversalGraph` based on a positional matrix (e.g., `[[a,b,c], [d,e,f]]`).

Matrices assume uniform grids. If a `nest` Layout contains irregular `dock` Layouts (e.g., Row 1 has 3 buttons, Row 2 has 2 buttons), navigating `SOUTH` from the third button in Row 1 results in an `IndexError`.

Traversal generation cannot be matrix-index based. It must be spatial. During `Layout` generation, after all absolute `(x, y)` pixels are calculated, cast a spatial ray (AABB projection) from each `Button`'s center in the four cardinal directions to find the nearest intersecting `Button` neighbor.
...
* [ ] *Widget Frames* Implement Widget frames.
    ...
    * [x] Implement `DynamicFrame`: Bypasses Registry string lookups and instructs the renderer to pull `TexturePtr` directly from the `DisplayState.canvas`.
...
* [ ] *Z-Sorting Enforcement:* Assign `state.height` and `state.depth` modifiers to embedded Icons and Decals o ensure strict Painter's Algorithm compliance during flattening.
...
`AnimationMechanics` strictly governs "World Time". `MenuMechanics` must govern "UI Time". The `animate()` interface for Widgets must be called inside `MenuMechanics.update()`, iterating over `board.overlays` (always) and `board.menus[-1]` (if active).
